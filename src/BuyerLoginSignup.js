...
import './BuyerLoginSignup.css';

function BuyerLoginSignup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin) {
      ...
      if (formData.address.trim().length < 5) {  // Validate address length
        setPopupMessage('Address must be at least 5 characters long.');
        setShowPopup(true);
        return;
      }
    }
    ...
    try {
      if (isLogin) {
        ...
        if (response.data === 'Login successful') {
          // Redirect to buyer homepage/dashboard
          navigate('/buyer-home');
        }
      } else {
  ...
  return (
    <div className="buyer-auth">
      <h1>{isLogin ? 'Buyer Login' : 'Buyer Signup'}</h1>
...
export default BuyerLoginSignup;
